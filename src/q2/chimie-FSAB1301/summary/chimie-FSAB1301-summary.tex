...
% - le commit d616268 a créé un nouveau dossier chimie-FSAB1301 en laissant l'ancien dossier chimie à côté (ce qui fait que Git n'a pas fait le lien)
% - le commit c6a3999 a supprimé l'ancien dossier, et a cassé l'historique du fichier
% Si vous voulez retrouver l'historique du fichier avant ces commits, afficher l'état du dépôt au commit d616268.
...
Énergie de réaction: énergie absorbée par le système (par convention)
...
%FIXME : normalement ça constitue des parenthèses à ne pas connaitre.
...
à une couche à une distance $n_f$ vaut
...
Ces transitions expliquent les spectres d'absorption et d'émission des éléments.
...
Max Born: $\abs{\psi^2} = $ densité de probabilité de présence de la particule (probabilité de présence/volume).
...
$n$ détermine la couche de l'orbitale, et peut être notée par une lettre majuscule (à partir de K, \textit{kernel}). Enfin, tous les éléments d'une période $n$ dans le tableau périodique ont le même nombre de couches électroniques, à savoir $n$.
%La valeur de $n$ peut aussi être représenté par la période correspondante
% NOT SURE
% En effet, ce n'est pas vraiment ça ; mais numéro de la période = numéro de la couche de valence (selon les définition données du prof).
\begin{center}
...
$l$ détermine la forme de l'orbitale et la sous-couche.
Il y a exactement $n$ formes possible pour une orbitale de nombre quantique $n$, et
$l$ peut prendre les valeurs $0, 1, \ldots, n-1$ (en commençant à zéro).
...
\begin{itemize}
  \item État fondamental: l'$e^-$ occupe une orbitale 1s
...
On entend par atomes hydrogénoïdes, ou atomes monoélectroniques, les atomes ou ions qui possèdent exactement $1$ électron (par exemple: \ce{H}, \ce{He^+}, \ce{O^7+}, ...).
...
\begin{itemize}
  \item Écrantage: Chaque électron est blindé contre l’attraction complète du noyau par les couches inférieures d'électrons (répulsions interélectroniques)
  ...
  \item Énergie fonction de $n$ et de $l$.
\end{itemize}
...
Exemple : le fluor \ce{F} a pour configuration électronique \configElectronique{9} ou \configElectronique[courte]{9}.

\subsection{Exceptions}
Les atomes du groupe 11 (cuivre, argent, or) complètent leur sous-couche $\dorb$ avec un des électrons de la sous-couche $\sorb$ externe, en désaccord avec Klechkowsky (où la sous-couche $\sorb$ devrait être remplie), car cette configuration est plus stable énergétiquement.
Ainsi, au lieu d'être \configElectronique[courte]{29}, le cuivre a pour configuration électronique $\left[\text{Ar}\right] \, \text{3d}^\text{10} \, \text{4s}^\text{1}$.% j'ai adapté le code du fichier annexe, peut-être l'améliorer ?
Conséquence : les liaisons chimiques de ces éléments impliquent principalement l'orbitale $\sorb$ et beaucoup moins l'orbitale $\dorb$, ce qui diminue le caractère covalent des liaisons et augmente la mobilité des électrons et donc, la conductivité.

Ce phénomène se retrouve aussi dans le cas du chrome, qui complète sa demi-sous-couche $3\dorb$ au lieu de la sous-couche $4\sorb$ ; sa configuration électronique est $\left[\text{Ar}\right] \, \text{3d}^5 \, \text{4s}^1$ au lieu de \configElectronique[courte]{24}. Dans les périodes $\ge 5$, la majorité des métaux de transition et les éléments du bloc $\forb$ constituent des exceptions (qu'il ne faut pas connaitre).

\section{Électrons de valence et électrons de cœur}

Il existe plusieurs manières de définir les électrons de cœur et les électrons de valence.
Si ces définition sont identiques pour les atomes des 3 premières périodes, elles divergent pour les suivantes et sont moins pertinentes.

Définition dans le cadre du cours :
\begin{description}
	\item[Électrons de cœur]
	  Électrons se trouvant dans les orbitales du gaz rare précédent.
	\item[Électrons de valence]
	  Électrons se situant dans la dernière couche électronique partiellement ou totalement remplie.
\end{description}

Exemple : Pour le scandium, de configuration \configElectronique{21}, les électrons de cœur sont \configElectronique{18}, les électrons de valence $\text{4s}^2$ et l'électron de la sous-couche $3\dorb$ n'est ni l'un ni l'autre. Néanmoins, il vaut mieux préciser la définition que l'on applique, voire d'énoncer les deux possibilités : les électrons de valence du scandium sont $\text{4s}^2$ si on considère que les électrons de valence sont ceux de la couche externe, ou $\text{3d}^1\text{4s}^2$ si on tient aussi compte des électrons de la sous-couche $\dorb$.

Les électrons des sous-couches $\dorb$ peuvent participer à des liaisons chimiques (paragraphe~\ref{subpar:valence_d}).
...
\begin{itemize}
  \item Toujours endothermique (donc $>0$);
  \item Aux exceptions près, $E_I$ est inversement proportionnel au rayon atomique : plus celui-ci est élevé, plus l'électron arraché est loin du noyau, et donc moins l'énergie d'ionisation est grande. $E_I$ diminue en descendant dans un groupe car l'électron ionisé, sur une couche de plus en plus éloignée du noyau, est de moins en moins lié ; et $E_I$ augmente le long d'une période à cause de l'augmentation de la charge nucléaire effective ;
  \item Le passage d'une sous-couche à une autre d'énergie supérieure, ou dans une couche supérieure, s'accompagne d'une diminution de l'énergie d'ionisation (exemple : passage béryllium $\rightarrow$ bore : l'électron $\text{3p}$ est moins lié que le $\text{3s}$) ;
  \item La répulsion entre 2 $e^-$ appariés (d'une même orbitale) augmente leur énergie et rend le départ d'un électron plus facile que dans le cas de 2 $e^-$ non-appariés (exemple : passage azote $\rightarrow$ oxygène) ;
  ...
  \item Rayon cationique < Rayon atomique ; les ionisations successives ont un rayon de plus en plus petit (moins d'$e^-$ et moins de répulsions).
\end{itemize}

\section{Électroaffinité}
\label{sec:electro}

$E_A$ = énergie absorbée lors de la capture d'un électron par l'atome \textbf{en phase gazeuse}.%FIXME Nysten dit absorbée, Atkins dégagée, et les tables absorbée.
\[ \ce{X(g) + e- -> X^-(g)} \]
...
\begin{itemize}
  ...
  \item $E_A$ diminue lorsque l'électron capturé doit prendre place dans une orbitale de plus grande énergie, comme une orbitale déjà occupée (cas de l'azote, où l'électron doit s'apparier) ou une nouvelle sous-couche (cas du béryllium).
...
\begin{itemize}
  ...
  \item liaison \emph{forte} et \emph{isotrope} (identique dans toutes les directions).
\end{itemize}
...
\begin{center}
  \begin{tabular}{llll}
    Énergie d'ionisation & $\ce{M -> M+ + e-}$ & $\Delta E = E_I > 0$ & endo\\
    Électroaffinité & $\ce{X + e- -> X-}$ & $\Delta E = E_A < 0$ & exo\\
    Énergie de neutralisation & $\ce{M+ + X- -> M+X-}$ & $\Delta E = E_N < 0$ & exo
  \end{tabular}
\end{center}
...
\label{sec:neutral}
L'énergie de neutralisation est l'énergie à fournir pour lier deux atomes déjà ionisés. $d = r_{\mathrm{anion}} + r_{\mathrm{cation}}$ la distance entre les centres atomiques.

\subparagraph{Pour les gaz}
\[ E_N = \frac{-N_A}{4\pi\epsilon_0}\times\frac{|z_1z_2|e^2}{d} \]
où $z_1$ et $z_2$ sont les valences des atomes
et $e$ la charge d'un électron.
...
$A$ est la constante de Madelung qui varie selon les composés.

\paragraph{Calcul de l'enthalpie de formation et de l'enthalpie réticulaire d'un solide : cycle de Born-Haber}
%\begin{figure}
%	\centering
%	\begin{tikzpicture}
%	%TODO
%	\end{tikzpicture}
%	\caption{Cycle de Born-Haber}
%	\label{fig:bornhaber}
%\end{figure}

L'énergie (ou enthalpie) réticulaire est l'énergie de formation d'un cristal solide (par exemple de $\ce{NaCl(s)}$) à partir des constituants à l'état gazeux (ici, $\ce{Na(s)}$ et $\ce{Cl(g)}$). Pour la calculer, on utilise un cycle de Born-Haber, qui décompose la réaction en plusieurs petites réactions dont les énergies sont connues ou plus facilement calculables. Plus précisément, il faut souvent calculer
\begin{itemize}
	\item l'énergie de vaporisation ou de sublimation des constituants (car ils doivent être à l'état gazeux) (par exemple, $\ce{Na(s) -> Na(g)}$) ; ce n'est pas nécessaire si l'on demande l'énergie réticulaire, car les constituants sont par définition à l'état gazeux ;
	\item l'énergie de dissociation des molécules covalentes (ici, $\ce{Cl2}$) ;
	\item l'énergie d'ionisation du cation ($\ce{Na+}$) et l'affinité électronique de l'anion ($\ce{Cl-}$) ;
	\item l'énergie de neutralisation, pour la formation du solide ionique ($\ce{Na+(g) + Cl-(g) -> NaCl(s)}$). On utilise l'expression pour les solides ioniques.
\end{itemize}
Une fois toutes ces valeurs calculées, on les ajoute et cela nous donne l'énergie de formation du cristal.

\subparagraph{Exemple} Calculons l'enthalpie de formation de $\ce{CsI(s)}$ à partir de césium liquide et d'iode solide. La réaction est $\ce{Cs(l) + 1/2I2(s) -> CsI(s)}$.
On calcule :
\begin{itemize}
	\item $E_1=\Delta_\mathrm{vap} H_{\ce{Cs}} = \numprint[\kilo\joule\per\mole]{+67.74}$ ;
	\item $E_2=\frac{1}{2} \Delta_\mathrm{subl} H_{\ce{I2}} = \frac{1}{2} \numprint[\kilo\joule\per\mole]{+62.42} = \numprint[\kilo\joule\per\mole]{+31.21}$ ;
	\item $E_3=\frac{1}{2} E_\mathrm{diss}^{\ce{I2}} = \frac{1}{2} \numprint[\kilo\joule\per\mole]{+139} = \numprint[\kilo\joule\per\mole]{+69.5}$ ;
	\item $E_4=E_I^{\ce{Cs}} = \numprint[\kilo\joule\per\mole]{+376}$ ;
	\item $E_5=E_A^{\ce{I2}} = \numprint[\kilo\joule\per\mole]{-295}$ ;
	\item $E_6 = E_N = \numprint[\kilo\joule\per\mole]{-576}$ (avec $d=167+220=\numprint[\pico\meter]{387}$, $z_1=z_2=1$ et $A=1.763$).
\end{itemize}
Et donc, l'enthalpie de formation est $\Delta H_f = E_1+E_2+E_3+E_4+E_5+E_6=\numprint[\kilo\joule\per\mole]{-327}$.
...
\begin{itemize}
  ...
  \item rigidité et fragilité des solides covalents;% heu, et le diamand ?
...
\label{subpar:valence_d}
Certains atomes peuvent loger plus de 8 $e^-$ dans leur couche de valence. Il s'agit des non-métaux de la 3\up{e}
période et suivantes. Les atomes de ces éléments possèdent des orbitales $d$ vides,
ce qui leur permet d'étendre leur couche de valence.
...
Une molécule est polaire ssi la résultante de ses moments dipolaires est non nulle.
Pour cela, il faut faire une somme vectorielle pondérée des moments dipolaires.
...
\paragraph{Détermination du type de liaison}

\begin{center}
  \begin{tabular}{l|ll}
    Liaison & Nombre de liaisons $\sigma$ & Nombre de liaisons $\pi$\\
    \hline
    Simple & 1 & 0\\ %FIXME sauf $p-p$ où c'est $\pi$ <- not sure
    Double & 1 & 1\\
    Triple & 1 & 2\\
  \end{tabular}
\end{center}
...
On va créer une orbitale intermédiaire $\sorb\porb^3$ dont le niveau énergétique est supérieur à celui de $\sorb$ et inférieur à celui de $\porb$.
...
\subsection{Configuration spatiale des molécules}

La théorie VSEPR permet de déterminer dans bien des cas la forme de la molécule, en particulier l'agencement des atomes les uns par rapport aux autres, et donc de déterminer si certains atomes sont dans le même plan ou non, ou leur nombre. Généralement, on cherche à compter le nombre d'atomes dans le plan contenant l'axe principal de la molécule (parfois, il y en a plusieurs). Quelques éléments permettant de répondre à la question :
\begin{itemize}
	\item dans les configurations triangulaires planes et linéaires, les atomes de ces configurations sont tous dans le même plan. Par exemple, dans le cas de l'éthène, les deux atomes de carbone et les deux atomes hydrogène liés à l'un d'eux sont dans le même plan ;
	\item les configurations tétraédriques ne permettent pas d'avoir tous les atomes dans le même plan : dans le meilleur des cas, seuls trois atomes (centre+deux liés) sont dans un même plan ;
	\item les doubles liaisons bloquent la rotation des atomes autour de l'axe de la liaison, et donc la rotation des plans ;
	\item dans le cas d'un atome à deux doubles liaisons (donc 4 ; linéaire), les doubles liaisons sont perpendiculaires, et les plans des liaisons restantes sont aussi perpendiculaires.
\end{itemize}

Quelques exemples (tirés d'examens) :%FIXME trouver d'autres exemples.
\begin{itemize}
	\item La molécule d'acrylonitrile \ce{H2C-CHCN} a tous ses atomes dans le même plan : le carbone de gauche est en triangulaire, le C de droite aussi, la double liaison force les plans à rester alignés, et le carbone lié à l'azote est linéaire, et est donc dans le même plan.
	\item La molécule \ce{H2C=CH2} a tous ses atomes dans le même plan (deux configurations triangulaires reliées par une double liaison), la molécule \ce{H2C=C=CH2} possède deux plans perpendiculaires (les deux doubles liaisons du carbone central sont perpendiculaires), et la molécule \ce{H2C=C=C=CH2} a tous ses atomes dans le même plan.
	\item L'acide acrylique \ce{H2C=CH-COOH} possède 6 atomes dans le même plan : elle n'en possède pas plus car il y a possibilité de rotation autour de la liaison C-C simple.
\end{itemize}
...
Les températures de fusion et d'ébullition augmentent avec leur nombre et leur intensité.
...
\begin{itemize}
  ...
  \item Ions-dipôles : explique le pouvoir de solvatation de l'eau: les molécules d'eau se fixent à chaque ion et les séparent des autres ions $\rightarrow$ hydratation due au caractère polaire de \ce{H2O} (attirance charges de signe opposé).
    Les interactions ion-dipôle sont fortes pour des petits ions très chargés. $E_p \propto \frac{-|z| \mu}{r^2}$, où $|z|$ est la charge de l'ion et $\mu$ est la polarisation du dipôle.
...
Molécules polaires. $\mu$ est l'intensité du moment dipolaire.

\paragraph{Dipôle induit}
Un dipôle induit est un dipôle qui se forme dans une molécule lorsqu'une autre molécule polaire ou un ion est à proximité. $\alpha$ est la fonction de polarisabilité.

\subsection{Forces de London}
Dipôle induit-induit.

\[ E_p \propto \frac{-\alpha_1 \alpha_2}{r^6} \]
...
il y a une attraction permanente entre les 2 molécules.

\subsection{Forces de Debye}
Dipôle permanent-induit.

\[ E_p \propto \frac{-\mu_1^2 \alpha_2}{r^6} \]
...
l'intensité de l'interaction dépend de la polarisabilité de la molécule.

\subsection{Forces de Keesom}
Dipôle permanent-permanent.

\[ E_p \propto \frac{-\mu_1^2 \mu_2^2}{r^6} \]
...
\begin{itemize}
  \item Intensité du moment dipolaire ($\mu$) : augmente avec la différence d'électronégativité $\Delta\chi$ et avec la taille des molécules;
  \item Polarisabilité : $\alpha$ augmente avec le nombre d'$e^-$ et avec le rayon atomique
...
\paragraph{Origine}
Les atomes $\ce{H}$ de petites tailles,
partiellement chargé $\delta^+$, se rapprochent des atomes ($\ce{F}$, $\ce{O}$, $\ce{N}$) fortement électronégatifs, avec une charge $\delta^-$.
...
Le nombre de ponts H \emph{par molécules} est égal au minimum entre le nombre de \ce{H} liés à \ce{F}, \ce{O}, \ce{N}, et le nombre de doublets libres de ces atomes. Chaque molécule participe au double de ce nombre de ponts H.
\subparagraph{Exemples}
\ce{H2O} peut former, \emph{au total}, 4 ponts H, \ce{CH3COOH} peut en former 2 (entre l'oxygène doublement lié au \ce{C} et polarisé, et l'hydrogène lié à O), \ce{NH3} peut en former 2 et \ce{NH2OH} peut théoriquement en former 6.

\subsection{Influence des liaisons sur la température d'ébullition}

Afin de comparer la température d'ébullition de deux molécules différentes, il faut déterminer les intensités relatives des liaisons intermoléculaires des deux molécules.

Voici les différents facteurs qui influencent la température d'ébullition, par ordre décroissant d'importance (les premiers facteurs sont les plus différenciant) :
\begin{itemize}
	\item le type de liaisons : ioniques >> métalliques >> covalents.
	\item la masse molaire de la molécule : plus celle-ci est élevée, plus la température est élevée. Si la différence de masse molaire n'est pas significative (inférieure à 10\%), on ne peut pas conclure. Si l'une des molécules peut faire des liaisons hydrogène tandis que l'autre ne peut pas, il faut que la différence de masse molaire soit très grande pour que ce critère soit applicable.
	\item la présence de ponts hydrogène et leur multiplicité. C'est peut-être le critère le plus important pour des molécules de masse assez proche.
	\item le type de ponts hydrogènes : les ponts H-O sont plus intenses que les ponts H-N.
	\item la polarité de la molécule : elle augmente avec l'intensité de la polarisation (double, triple liaison ; différence d'électronégativité), le nombre de liaisons polaires, et l'alignement de la polarisation (par exemple, le méthoxyéthane $\ce{CH3-CH2-O-CH3}$ a une température d'ébullition de 7,4\degree C, l'acétone $\ce{CH3-CO-CH3}$ 56\degree C, et l'éthylène glycol $\ce{HO-CH2-CH2-OH}$ 197\degree C) ;
	\item le type de liaison van der Waals : plus la liaison est forte, plus les molécules sont liées et plus la température est élevée (Keesom > Debye > London) ;
	\item la symétrie de la molécule : celle-ci influence la polarisation de la molécule ;
	\item la longueur et la grandeur de la molécule : plus la molécule est longue ou vaste, plus les liaisons de van der Waals seront fortes.
\end{itemize}
...
Le modèle des gaz parfaits est un modèle simplifié des gaz réels.
Dans ce modèle, le gaz est constitué de molécules animées de mouvements
...
des pressions partielles dues à chaque gaz :
...
\subsection{Énergie d'un corps et première loi de la thermodynamique}

\subsubsection{Énergie interne d'un corps}
...
\subparagraph{Système ouvert} Échanges de matière et d'énergie avec l'environnement.
...
c'est à dire,
par mole d'un composé ayant un coefficient stœchiométrique 1 dans la réaction.
...
%TODO A garder mais en expliquant mieux.
...
des produits et des réactifs pondérées par leur coefficient stœchiométrique.
...
\paragraph{État de référence standard}
...
L'Équation de Gibbs et un peu d'algèbre nous permet de dresser la Table~\ref{tab:hsgt}.
\begin{table}[ht!]
  \begin{center}
    \begin{tabular}{|cccll|}
      \hline
...
réactifs (état initial), en tenant compte de la stœchiométrie de la réaction. % TOdo attention à ce que ça va donner
...
\paragraph{Équation de Clapeyron-Clausius}
...
\subsection{Équilibre chimique à température et pression totale constantes}
...
toujours le cas lorsque les coefficients stœchiométriques des composés de
...
Une base forte est un électrolyte fort $\rightarrow$ dissociation totale
en solution aqueuse.
...
\begin{enumerate}
	\item Écriture de l'équation d'équilibre des charges ;
	\item Écriture de l'équation de conservation de la matière ;
...
\section{Équilibre en phase aqueuse}
...
\begin{enumerate}
	...
	\item	Écrivez l’équation chimique équilibrée de la réaction de neutralisation et
...
\subsection{Neutralisation d'un acide fort (\ce{HCl}) par une base forte (\ce{NaOH)}}
...
\subsection{Équilibres de solubilité}
...
On va supposer ici que la quantité d'ions chlorure apportée par la dissolution du \ce{PbCl2(s)}
...
\annexe %FIXME : changer commande dans eplbase pour rester cohérent avec LaTeX ?
\section{Notations}
\label{ann:nota}
De nombreuses notations seront utilisées au cours de cette synthèse.
Pour des raisons de concision,
leur signification ne sera pas rappelée à chaque fois mais elle sera indiquée dans la Table~\ref{tab:notations}.
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{ll}
      ...
      $E_A$ & Électroaffinité d'un atome,
      voir section~\ref{sec:electro} page~\pageref{sec:electro}\\
      $E_I$ & Énergie d'ionisation d'un atome,
      voir section~\ref{sec:ioni} page~\pageref{sec:ioni}\\
      $E_N$ & Énergie de neutralisation entre deux atomes (liaison ionique),
      voir section~\ref{sec:neutral} page~\pageref{sec:neutral}\\
      $E_l$ & Énergie de liaison covalente,
...
Quand on travaille avec des énergies,
