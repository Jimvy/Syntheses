...
{\bf La numérotation se fait à partir de $0$}.
...
Les atomes du groupe 11 (cuivre, argent, or) complètent leur sous-couche $\dorb$ avec un des électrons de la sous-couche $\sorb$ externe, en désaccord avec la règle de Klechkowsky (où la sous-couche $\sorb$ devrait être remplie), car cette configuration est plus stable énergétiquement.
...
Cela a d'ailleurs une conséquence sur leur conductivité électrique : les liaisons chimiques de ces éléments impliquent principalement l'orbitale $\sorb$ et beaucoup moins l'orbitale $\dorb$, ce qui diminue le caractère covalent des liaisons et augmente la mobilité des électrons et donc, la conductivité.
...
Il existe plusieurs manières de définir ce que sont les électrons de cœur et les électrons de valence.
Si ces définition sont identiques pour les atomes des 3 premières périodes, elles divergent pour les suivantes et ont moins de pertinence.
...
Les électrons des sous-couches $\dorb$ peuvent participer aux liaisons chimiques, en particulier à des liaisons covalentes (paragraphe~\ref{subpar:valence_d}).
...
\begin{itemize}
	...
	\item l'énergie d'ionisation du cation ($\ce{Na -> Na+ + e-}$) et l'affinité électronique de l'anion ($\ce{Cl + e- -> Cl-}$) ;
	\item l'énergie de neutralisation, pour la formation du solide ionique ($\ce{Na+(g) + Cl-(g) -> NaCl(s)}$). On utilise la formule pour les solides ioniques.
\end{itemize}
...
\begin{itemize}
  ...
  \item Forces de Van der Waals
    ...
    \end{center}
\end{itemize}
%TODO modifier (c'est là où ça coinçait).
...
Un dipôle induit est un dipôle qui se forme dans une molécule lorsqu'une autre molécule polaire ou un ion est à proximité.
...
$\ce{O}$, $\ce{N}$, des atomes fortement électronégatifs.
Les atomes $\ce{H}$,
...
\begin{itemize}
	...
	\item la symétrie de la molécule : celle-ci influence la polarisation de la molécule, mais dans des proportions plus faibles.
