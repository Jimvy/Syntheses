...
Une EDP est dite \emph{homogène} quand elle ne contient que des termes ne faisant intervenir $u$ ou ses dérivées partielles. Par exemple, l'équation~\eqref{eq:intro-exemple1} est homogène si $F=0$. Une équation homogène admet toujours la solution nulle $u=0$. Une EDP linéaire mais non-homogène a comme solution générale la somme d'une solution particulière de l'équation non-homogène et de la solution générale de l'équation homogène correspondante, tout comme leurs cousins EDO.
...
\end{equation}
C'est une équation supplémentaire qui relie $\fpart{u}{x}$ et $\fpart{u}{y}$ ; on peut donc écrire, à partir de cette équation et notre EDP (équation~\eqref{eq:order1-genequa}), le système
\begin{equation}
...
Le système~\eqref{eq:order1-dirpart} ne doit pas être confondu avec le système~\eqref{eq:order1-bienpose-edp-duds}. Ce dernier exprime une condition sur $\Gamma$, telle que le problème est bien posée ; tandis que le système~\eqref{eq:order1-dirpart} permet de déterminer une direction locale intéressante, parallèle à $(P, Q)$. Cette direction n'est jamais parallèle à la direction de la courbe de Cauchy ; si tel était le cas, le déterminant de~\eqref{eq:order1-bienpose-edp-duds} serait nul, et le problème serait mal posé
\footnote{Si $(\dif{x}, \dif{y}) = k \cdot \left(\fdif{x}{s}, \fdif{y}{s}\right)$, autrement dit s'ils sont parallèles, alors le déterminant de~\eqref{eq:order1-bienpose-edp-duds} devient \[ \begin{vmatrix} P & Q \\ \fdif{x}{s} & \fdif{y}{s} \\ \end{vmatrix} = \frac{1}{k} \begin{vmatrix} P & Q \\ \dif{x} & \dif{y} \\ \end{vmatrix} =0 \] et donc, le problème serait déjà mal posé à la base.}.
...
Notons qu'une seule des deux relations de compatibilité est requise pour l'intégration ; l'autre relation peut directement être obtenue à partir de l'une et de~\eqref{eq:order1-caractode}. Une manière de retenir ces relations est
\begin{equation}
...
Une relation supplémentaire qui peut être utile est $\sqrt{P^2+Q^2} \dif{u} = R\dif{l}$; celle-ci relie la variation de $u$ avec la variation $\dif{l}$ directement le long de la caractéristique (dans l'axe de la caractéristique), de sorte que $\dif{l}^2 = \dif{x}^2 + \dif{y}^2$. La démonstration est simplement une règle de la chaine sur $\fdif{u}{l}$, avec quelques manipulations à partir des relation~\eqref{eq:order1-diffeqnmemo}.
...
On commence par déterminer le réseau des caractéristiques. Puisque $P=y$ et $Q=-x$, les caractéristiques sont déterminées par l'intégration suivante (équation~\eqref{eq:order1-caractcurve_integral}) :
\begin{align*}
P \dif{y} &= Q \dif{x} \\
y \dif{y} &= -x \dif{x} \\
...
Enfin, les dérivées de chaque terme du gradient, $\fpart{}{s} \left(\fpart{\phi}{x}\right)$ et $\fpart{}{s} \left(\fpart{\phi}{y}\right)$ sont également connus le long de $\Gamma$. On peut ainsi écrire (similairement à l'équation~\eqref{eq:order1-duds}) :
\begin{equation}
...
\end{equation}
et de la même manière, on peut tout mettre en notation matricielle (similaire à~\eqref{eq:order1-bienpose-edp-duds})
\begin{equation}
...
\end{align}
\end{subequations}
et donc, on peut propager la solution sur tout le plan. Pour que cela fonctionne, il faut que le déterminant de la matrice de~\eqref{eq:order2-bienpose-edp-dphidxys} ne s'annule pas -- cela définit un \emph{problème bien posé}.
...
\begin{itemize}
	\item Lorsque $B^2-4AC>0$, l'EDP est qualifiée \emph{hyperbolique}. L'équation~\eqref{eq:order2-caractequa} possède deux solution distinctes, et la méthode des caractéristiques est applicable. Un exemple est l'équation d'onde.
	\item Lorsque $B^2-4AC=0$, l'EDP est dite \emph{parabolique}. L'équation~\eqref{eq:order2-caractequa} ne possède qu'une seule solution, et la méthode des caractéristiques n'est pas applicable. Un exemple est l'équation de diffusion, qui doit être résolue en partant d'une condition initiale.
	\item Lorsque $B^2-4AC<0$, l'EDP est \emph{elliptique}. Les solutions de~\eqref{eq:order2-caractequa} sont complexes, la méthode des caractéristiques n'est plus applicable. Un exemple est l'équation de Poisson, qui doit être résolue de manière globale.
\end{itemize}
...
Dans le cas des EDP hyperboliques, l'équation~\eqref{eq:order2-caractequa} a deux solutions réelles, et définit donc deux directions caractéristiques. La méthode des caractéristiques est alors utilisable, et on peut propager la solution $u$ de la courbe de Cauchy sur l'ensemble du plan.

Les caractéristiques peuvent être obtenues en intégrant les deux EDO obtenues à partir de la relation caractéristique, d'une manière assez similaire à celle utilisée lors des EDP du 1\ier{} ordre. Ensuite, la valeur de $\fpart{\phi}{x}$ et $\fpart{\phi}{y}$ peuvent être obtenues en intégrant les relations obtenues en remplaçant le terme indépendant de~\eqref{eq:order2-dirpart} dans une colonne de la matrice. Cela fait donc deux EDO le long de deux caractéristiques. Enfin, la valeur de $\phi$ peut être évaluée à partir des relations~\eqref{eq:order2-propag-all}.
...
\end{align}
\end{subequations}
On constate que ce sont justement les deux opérateurs résultants de la factorisation, à un facteur $1/2c$ près. On peut donc réécrire l'équation~\eqref{eq:order2-ondes-facto} sous la \emph{forme canonique}
\begin{equation}
...
\Rightarrow g(y) &= l = k
\end{align*}
et donc, $f(x) = g(y) = k$. Par conséquent,  notre équation~\eqref{eq:separ_laplace_separated_one} se réécrit
...
&= A \sinh(kx) + B \cosh(kx)
\end{align*}
avec de nouvelles constantes. En pratique, on réécrira souvent la solution à l'équation~\eqref{eq:separ_laplace_Xcases_pkk} sous cette forme.
...
&= A \cos(kx) + B \sin(kx)
\end{align*}
avec de nouveaux noms de constantes. On écrira généralement la solution à l'équation~\eqref{eq:separ_laplace_Xcases_mkk} sous cette forme.
...
Nous avons donc, pour chaque $n$ et chaque $k_n$, une solution $X_n(x)$ à~\eqref{eq:separ_laplace_Xcases_mkk} qui vaut
\begin{equation}
...
Revenons à~\eqref{eq:separ_laplace_separated_lambda}, et passons à la résolution de l'équation en $Y$. Nous savons donc que
...
% Partie équation d'Euler

\subsubsection{L'équation d'Euler}

L'équation
\[ r^2 R''(r) + b r R'(r) + c R(r) = 0 \]
avec $r>0$, est nommé \emph{équation d'Euler}. Il s'agit d'une équation du second ordre à coefficients non constants ; aucune méthode de résolution générale ne marche pour ce type d'équation.

Ne nous laissons pas abattre et tentons de trouver une solution de la forme $R(r) = r^n$, avec $n$ à déterminer. Cette solution est totalement arbitraire, mais nous allons voir qu'elle fonctionne malgré tout. En injectant dans l'équation, on obtient
\begin{align*}
0 &= r^2 (r^n)'' + b r (r^n)' + c r^n \\
&= r^2 n (n-1) r^{n-2} + b r n r^{n-1} + c r^n \\
&= r^n \left( n(n-1) + b n + c \right) \\
0 &= n^2 + (b-1) n + c
\end{align*}
Et donc, $n$ doit être solution d'une équation du deuxième degré. Analysons les 3 connus.

\paragraph{2 racines réelles}
Cela survient si $\Delta = (b-1)^2 - 4c >0$. Dans ce cas, on a deux solutions réelles, $n_1$ et $n_2$, et la solution générale de l'équation d'Euler est
\[ R(r) = c_1 r^{n_1} + c_2 r^{n_2} \]
avec $c_1$ et $c_2$ deux constantes à déterminer avec les conditions limites.

\paragraph{1 racine double}
Cela survient quand $\Delta = (b-1)^2 - 4c = 0$. Dans ce cas, nous n'avons à priori qu'une seule solution $n_0=\frac{1-b}{2}$, or il nous en faut deux. La deuxième solution sera dans ce cas $R_2(r) = r^{n_0} \ln(r)$, ce qui donne la solution générale
\[ R(r) = c_1 r^{n_0} + c_2 r^{n_0} \ln(r) = r^{n_0} (c_1 + c_2\ln(r)) \]
Pourquoi $r^{n_0}\ln(r)$ ? Supposons pendant un instant que $R(r)=v(r) r^{n_0}$ soit une solution de l'équation, et regardons quelles conditions doivent être appliquées à $v(r)$ :
\begin{align*}
0 &= r^2 \Big(v(r) r^{n_0}\Big)'' + b r \Big(v(r) r^{n_0}\Big)' + c v(r) r^{n_0} \\
&= r^2 \Big( v''(r) r^{n_0} + v'(r) n_0 r^{n_0-1} + n_0 v'(r) r^{n_0-1} + n_0(n_0-1) v(r) r^{n_0-2} \Big) \\
	&\qquad + b r \Big( v'(r) r^{n_0} + v(r) n_0 r^{n_0-1} \Big)
	+ c v(r) r^{n_0} \\
&= v(r) r^{n_0} \Big( n_0(n_0-1) + b n_0 + c \Big)
	+ v'(r) r^{n_0+1} \Big( 2 n_0 + b \Big)
	+ v''(r) r^{n_0+2} \\
&= 0 + v'(r) r^{n_0+1} + v''(r) r^{n_0+2} \\
&= r^{n_0+1} \Big( v'(r) + r v''(r) \Big)
\end{align*}
ce qui implique que $v'(r) = -r v''(r)$, équation différentielle ordinaire séparable qui a comme solution $v(r) = c_1 \ln(r) + c_2$, ce qui confirme bien notre solution (aux constantes près).

\paragraph{2 racines complexes}
Pour ce dernier cas, supposons que nos solutions soient de la forme $n_1 = \lambda + i\mu$ et $n_2 = \lambda - i\mu$ ; c'est le cas si nos coefficients $b$ et $c$ sont réels. On a alors la solution générale
\begin{align*}
R(r) &= c_1 r^{\lambda + i\mu} + c_2 r^{\lambda - i\mu} \\
&= c_1 e^{(\lambda + i\mu)\ln(r)} + c_2 r^{(\lambda - i\mu)\ln(r)} \\
&= c_1 e^{\lambda\ln(r)} e^{i\mu\ln(r)} + c_2 e^{\lambda\ln(r)} e^{-i\mu\ln(r)} \\
&= c_1 r^\lambda (\cos(\mu\ln(r)) + i\sin(\mu\ln(r))) + c_2 r^\lambda (\cos(\mu\ln(r)) - i\sin(\mu\ln(r))) \\
&= r^\lambda \Big( (c_1 + c_2) r^\lambda\cos(\mu\ln(r)) + (c_1 - c_2)i r^\lambda\sin(\mu\ln(r)) \Big)
\end{align*}

Il est à noter que les développements ci-dessus restent valides pour $x<0$, en effectuant un changement de variable ; il suffit de remplacer $r$ par $\abs{r}$.
...
\begin{equation}
\phi_{n, m}(x, y) = X_n^*(x) Y_m^*(y) = \sin\left(\frac{n\pi x}{L}\right) \sin\left(\frac{m\pi y}{H}\right)
...
R(a) &= 0
\end{align*}
L'équation~\eqref{eq:wave_bessel_pre_equation} n'est pas l'équation d'Euler (à cause du $k^2 r^2$), mais elle porte un nom, lorsqu'elle est écrite sous une autre forme. En posant $z=kr$, on peut réécrire l'équation~\eqref{eq:wave_bessel_pre_equation} sous la forme %FIXME étapes du développement ?
\begin{equation}
...
Dans le cas où $r=m$, on a alors que $((r+1)^2-m^2)A_1 = (2m+1)A_1=0$, ce qui donne $A_1=0$, sauf si $m=\frac{-1}{2}$. Dans ce dernier cas, la relation~\eqref{eq:wave_bessel_frobenius_recurse} devient
...
\end{align*}
En réinjectant ces expressions dans l'équation~\eqref{eq:wave_bessel_frobenius_solpattern}, on obtient
\begin{align*}
...
Dans le cas où $m\neq \pm\frac{1}{2}$ (mais on considère encore que $r=m$), on a nécessairement $A_1=0$, et comme l'équation~\eqref{eq:wave_bessel_frobenius_recurse} est toujours valide, on a
\begin{align*}
...
\end{align*}
ce que l'on peut encore simplifier en utilisant l'équation~\eqref{eq:wave_bessel_recursive_derivative} :
\begin{align}
